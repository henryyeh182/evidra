/**
 * Walk the Garmin source-schema scenarios and print what each export shape
 * turns into.
 *
 *   npm run simulate:garmin
 *   npm run simulate:garmin -- --asOf 2026-08-01
 ...
 * given Garmin shape yields, which it cannot, and that a decision still lands
 * and explains itself either way. The same runner backs
 * eval/test/garminScenarios.test.js, so what prints here is what is asserted.
 */

import { GARMIN_SCENARIOS } from "../eval/scenarios/garmin.js";
import { runGarminScenario, DEFAULT_AS_OF } from "../eval/scenarios/run.js";
...
const scenarios = args.scenario
  ? GARMIN_SCENARIOS.filter((scenario) => scenario.id === args.scenario)
  : GARMIN_SCENARIOS;

if (scenarios.length === 0) {
  console.error(`Unknown scenario: ${args.scenario}`);
  console.error(`Available: ${GARMIN_SCENARIOS.map((s) => s.id).join(", ")}`);
  process.exit(1);
}

const results = [];
for (const scenario of scenarios) {
  results.push(await runGarminScenario(scenario, { asOf: args.asOf }));
}
