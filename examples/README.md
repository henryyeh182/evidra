...
> **數字取自目前建置，不是可安裝的 v0.3.7。** 把這裡的樣本原封不動傳進去，
> v0.3.7 會給出相同結果——差別在**你的 AI 助理在 v0.3.7 上湊不出第 3 則的證據**：
> `vendorAssessments` 不在該版的 tool schema 裡，助理無從得知那個欄位存在，
> 只會送原始訊號。決策一樣是 `adjust`、一樣是 EVD-R-002，**信心會是 medium 而不是 high**。
> 其餘四則兩個版本一致。完整差距表在
> [user-journey.html](../docs/user-journey.html) 的〈Install〉一節。

**這裡的 evidence 是編造的樣本，用來示範形狀。** 決策與理由不是——
那些是這些輸入實際產生的輸出。真正使用時證據來自使用者自己的匯出檔或他說的話。
