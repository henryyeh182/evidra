...
import { normalizePrivateDir } from "./localEvidence.js";

const configuredDbPath = process.env.PACEVERA_DB_PATH
  ? (process.env.PACEVERA_DB_PATH === ":memory:"
    ? ":memory:"
    : isAbsolute(process.env.PACEVERA_DB_PATH)
      ? process.env.PACEVERA_DB_PATH
      : join(homedir(), process.env.PACEVERA_DB_PATH))
  : join(homedir(), ".pacevera", "pacevera.sqlite");
...
const localEvidenceDir = normalizePrivateDir(process.env.PACEVERA_PRIVATE_DIR);
