# Fitness MCP — Implementation Plan：決策日誌與查證存檔（v8 凍結）

> **2026-08-07 起，本檔不再是現況正本。** 現況與下一步移到
> [fitness-mcp-implementation-plan.md](fitness-mcp-implementation-plan.md)——那份只留
> 「現在是什麼狀態」與「下一步做什麼」。**本檔保留 v8 的完整推理過程**：
> 每個 Go/No-Go 怎麼判的、每條護城河的論證、通路盤點的原文引述、Phase 4/5 的負面結果、
> 4.5 待決事項、4.6 併入處置、§5 決策日誌、§6 風險——**這些東西不會過期成雜訊，
> 是回答「當初為什麼這樣定」唯一的地方，所以整份原樣保留，不摘要、不刪減。**
>
> 拆分理由：v8 累積到 1558 行，同時扛「決策稽核軌跡」與「下一步清單」兩個角色，
> 讓後者越來越難找。拆分只是把兩個角色分開放，**沒有任何一句話被判定為錯而移除**——
> 移到這裡純粹是「這是為什麼，不是現在要做什麼」。
>
> 之後要找「這個決定當初的理由／出處」，先查這裡的目錄（Ctrl+F 決策名或關鍵詞）；
> 要找「現在該做什麼」，去新的 plan。**新 plan 裡每個現況項目，出處都指得回這裡。**
...
- 缺資料時回 `signalCoverage.recovery.missing`／`signalCoverage.training.missing` 與降低 confidence，不補造數值。
...
**改為**：承諾 B 的衡量方式從「模型對照」改成「**決策可驗證性**」——決策規則是確定性的，正確與否由測試直接驗證（`assertValidDecision` ＋ 262 個測試）。
