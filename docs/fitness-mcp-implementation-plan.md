...
- **Story 4**：同一頁新增 prototype-only Outcome controls：`Adopted`、`Changed`、`Skipped` 與 perceived-effort slider。互動回饋明確寫出 `not saved`，不冒充 durable outcome persistence、account 或 hosted service。外部／內部 reviewer sessions 尚未完成，review protocol 與 findings template 見 [`docs/phase-0.5-review.md`](phase-0.5-review.md)。
