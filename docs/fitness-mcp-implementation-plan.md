...
| D-RULESCHEMA | ✅ **已實作**（`packages/rules` v1.2.0，9 條規則，見上「對外元件」）。2026-08-07 做過一次出處覆核：Gabbett 升為主文驗證，Mujika 撤回一組查不到的百分比並降級證據等級，ACSM 撤回一句摘要沒講的話。同日做 R3／R5：證據拆兩軸、`verificationStatus` 強制必填（見 §0）。2026-08-08 補上 `contested`，當場又撤回兩句（見 C11） |
