...
253 tests pass，全部 dependency-free（Node 20+，無外部套件）。
...
**改為**：承諾 B 的衡量方式從「模型對照」改成「**決策可驗證性**」——決策規則是確定性的，正確與否由測試直接驗證（`assertValidDecision` ＋ 253 個測試）。
...
| **保證** | ✅ 是，且更難複製。同證據永遠同決策，理由綁回證據，輸出帶 confidence／signalCoverage／limits | 已驗證（G3 gate、253 tests） |
...
- **Phase 9 驗收**：
  ...
  5. 既有 253 tests 全綠，且新增 dual-era 雙路徑測試
